<!-- header.phpを読み込む -->
<?php get_header(); ?>

<main>

    <!-- いったんコンテナーに格納 -->
    <div class="container ">



        <!-- 帯になるコンテナーのタイトル -->
        <div class="container">



            <!-- 帯の部分 -->
            <h2 class="h2__title mb20 fs35 bgPK">ドッグカフェ一覧</h2>

            <!-- パンくずリスト -->
            <div class="Breadcrumb mb20">
                <?php get_template_part("template-parts/breadcrumb"); ?>
            </div>


            <!-- コンテンツのサブタイトル -->

            <!-- 徳島市リスト-->
            <h3 class="tbl__center">徳島市</h3>
            <div class="tbl__border"></div>

            <div class="tbl__center">

                <div class="tbl__border"></div>
                <div class="tbl__sp"></div>
                <div class="tbl__main">
                    <table class="tbl__li">
                        <tr>
                            <th>名称</th>
                            <th>住所</th>
                            <th>お問い合わせ
                            </th>
                        </tr>
                        <tr class="line__solid">
                        </tr>

                        <tr>

                            <td><a href="">徳島●●</a></td>
                            <td>徳島市犬町わんこ番地12345</td>
                            <td>000-000-0000</td>
                        </tr>
                        <tr class="line__dashed">
                        </tr>
                        <tr>
                            <td><a href="">●●●●●●徳島●●</a></td>
                            <td>徳島市犬町わんこ番地12345</td>
                            <td>000-000-0000</td>
                        </tr>
                        <tr class="line__dashed">
                        </tr>

                        <tr>
                            <td><a href="">●●●●●●●●●●●●●●</a></td>
                            <td>徳島市犬町わんこ番地12345678910</td>
                            <td>000-000-0000</td>
                        </tr>
                        <tr class="line__dashed">
                        </tr>
                    </table>
                </div>
                <div class="list__sp"></div>
            </div>


        </div>
        <!-- 県東部リスト-->
        <h3 class="tbl__center">県東部</h3>
        <div class="tbl__border"></div>

        <div class="tbl__center">


            <div class="tbl__border"></div>
            <div class="tbl__sp"></div>
            <div class="tbl__main">
                <table class="tbl__li">
                    <tr>
                        <th>名称</th>
                        <th>住所</th>
                        <th>お問い合わせ
                        </th>
                    </tr>
                    <tr class="line__solid">
                    </tr>

                    <tr>

                        <td><a href="">徳島●●</a></td>
                        <td>徳島市犬町わんこ番地12345</td>
                        <td>000-000-0000</td>
                    </tr>
                    <tr class="line__dashed">
                    </tr>
                    <tr>
                        <td><a href="">●●●●●●徳島●●</a></td>
                        <td>徳島市犬町わんこ番地12345</td>
                        <td>000-000-0000</td>
                    </tr>
                    <tr class="line__dashed">
                    </tr>

                    <tr>
                        <td><a href="">●●●●●●●●●●●●●●</a></td>
                        <td>徳島市犬町わんこ番地12345678910</td>
                        <td>000-000-0000</td>
                    </tr>
                    <tr class="line__dashed">
                    </tr>
                </table>
            </div>
            <div class="list__sp"></div>
        </div>

        <!-- 県西部リスト-->
        <h3 class="tbl__center">県西部</h3>
        <div class="tbl__border"></div>

        <div class="tbl__center">
            <div class="tbl__border"></div>
            <div class="tbl__sp"></div>
            <div class="tbl__main">
                <table class="tbl__li">
                    <tr>
                        <th>名称</th>
                        <th>住所</th>
                        <th>お問い合わせ
                        </th>
                    </tr>
                    <tr class="line__solid">
                    </tr>

                    <tr>

                        <td><a href="">徳島●●</a></td>
                        <td>徳島市犬町わんこ番地12345</td>
                        <td>000-000-0000</td>
                    </tr>
                    <tr class="line__dashed">
                    </tr>
                    <tr>
                        <td><a href="">●●●●●●徳島●●</a></td>
                        <td>徳島市犬町わんこ番地12345</td>
                        <td>000-000-0000</td>
                    </tr>
                    <tr class="line__dashed">
                    </tr>

                    <tr>
                        <td><a href="">●●●●●●●●●●●●●●</a></td>
                        <td>徳島市犬町わんこ番地12345678910</td>
                        <td>000-000-0000</td>
                    </tr>
                    <tr class="line__dashed">
                    </tr>
                </table>
            </div>
            <div class="list__sp"></div>
        </div>

        <!-- 県南部リスト-->
        <h3 class="tbl__center">県南部</h3>
        <div class="tbl__border"></div>

        <div class="tbl__center">

            <div class="tbl__border"></div>
            <div class="tbl__sp"></div>
            <div class="tbl__main">
                <table class="tbl__li">
                    <tr>
                        <th>名称</th>
                        <th>住所</th>
                        <th>お問い合わせ
                        </th>
                    </tr>
                    <tr class="line__solid">
                    </tr>

                    <tr>

                        <td><a href="">徳島●●</a></td>
                        <td>徳島市犬町わんこ番地12345</td>
                        <td>000-000-0000</td>
                    </tr>
                    <tr class="line__dashed">
                    </tr>
                    <tr>
                        <td><a href="">●●●●●●徳島●●</a></td>
                        <td>徳島市犬町わんこ番地12345</td>
                        <td>000-000-0000</td>
                    </tr>
                    <tr class="line__dashed">
                    </tr>

                    <tr>
                        <td><a href="">●●●●●●●●●●●●●●</a></td>
                        <td>徳島市犬町わんこ番地12345678910</td>
                        <td>000-000-0000</td>
                    </tr>
                    <tr class="line__dashed">
                    </tr>
                </table>
            </div>
            <div class="list__sp"></div>
        </div>

        <!-- もどるボタン -->

        <div class="modoru__btn mb60">
            <a href="<?php echo home_url('/eat_type/cafe/'); ?>"><img src="<?php echo get_template_directory_uri(); ?>/assets/img/common/btn_modoru..svg" alt="もどる"></a>
        </div>

    </div>

</main>

<!-- footer.phpを読み込む -->
<?php get_footer(); ?>